...
class NewCR:
	...
	def option(self):
		...
		if self.yesno(question):
			shutil.copyfile(self.templateDir+'\\Actual Files\\HPL_delivery format_v3.19.xls', self.crDir + '\\HPL_delivery_template.xls')
		...
		if self.yesno(question):
			shutil.copyfile(self.templateDir+'\\startCR\\CR Toolkit.xlsx',crDir + '\\' + self.crName + ' Toolkit.xlsx')
			if os.path.isfile(crDir + '\\' + self.crName + ' Toolkit.xlsx'):
		...
		if self.yesno(question):
			shutil.copyfile(self.templateDir+'\\Actual Files\\GRSHFM_Metadata_v16110102.xlsb',crDir + '\\GRSHFM_Metadata_v16110102.xlsb')
			if os.path.isfile(crDir + '\\GRSHFM_Metadata_v16110102.xlsb'):
	...
	def templatecreate(self):									# Funct to create start templates(TD,FD,TE) for CR
		...
		shutil.copyfile(self.templateDir+'\\startCR\\TE_v1.00.docx',self.workDir + '\\' + self.crName+ '\\'+'TE_CR_' + self.crName[-2:] + '_v1.00.docx')
		...
		shutil.copyfile(self.templateDir+'\\startCR\\FD_v1.00.docx',self.workDir + '\\' + self.crName+ '\\'+'FD_CR_' + self.crName[-2:] + '_v1.00.docx')
		...
		shutil.copyfile(self.templateDir+'\\startCR\\TD_v1.00.docx',self.workDir + '\\' + self.crName+ '\\'+'TD_CR_' + self.crName[-2:] + '_v1.00.docx')
...
if __name__ == '__main__':
	...
	if config['userlogin'] != os.getlogin() or config['userlogin'] == "":
		...
		question = input('Do you want to use default folder for your CR\'s? \nIt will be {}: '.format(os.path.abspath(os.getenv('USERPROFILE')+'\\Documents\\CRs')))
